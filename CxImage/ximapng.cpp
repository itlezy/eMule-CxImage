...
/*
https://blog.birost.com/a?ID=00700-339d9764-04a3-435f-b4e8-8b14662490e0

1.2.6 There is no jmpbuf problem in png_struct_def structure
It means that the modification just now takes effect, but there is no jmpbuf field in png_struct_def. This may be caused by inconsistent versions.
In my version of libpng, there is no jmpbuf field, but it provides a function interface to get jmpbuf from png_struct_def

jmp_buf png_jmpbuf( png_structp *png_ptr);

Use png_jmpbuf(png_ptr) to replace the direct access to png_ptr->jmpbuff.
*/
